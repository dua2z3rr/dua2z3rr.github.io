...
Analizziamo l'output: è aperta solo la porta 80 con un server http con versione `HttpFileServer httpd 2.3`. Il sistema operativo dell'host è Windows, e i metodi supportati interessanti sono GET e POST.
...
Vediamo la possibilità di fare login sul server. Questo potrebbe esserci utile in futuro, tuttavia non abbiamo le credenziali.

### Ricerca Exploit

Cerchiamo un exploit per `HttpFileServer 2.3`

![Desktop View](/assets/img/optimum/optimum-hfs-server-exploit.png)

Questo Exploit (CVE-2014-6287) fa al caso nostro. Cerchiamo se esiste su metasploit.
...
Per il momento, teniamoci il payload di default. Se l'exploit non funziona, lo cambieremo.
...
Cerchiamo vie per diventare **System**.

Avviamo uno script di metasploit per fare ricerche di potenziali vettori di attacco per effettuare una privilege escalation.
...
Proviamo l'exploit trovato numero 5 (Ho scelto questo perchè il nome conteneva le parole **privilege escalation**).
